...
Plugin URI: http://wordpress.org/extend/plugins/esc-additional-post-thumbnails/
Description: Adds the ability to add multiple post thumbnails to a post type.
Version: 1.6.6
Author: Chris Scott
Author URI: http://voceplatforms.com/
*/
if (!class_exists('AdditionalPostThumbnails')) {

	class AdditionalPostThumbnails {
		...
		public function register($args = array()) {
			...
			if (null === $this->label || null === $this->id) {
				if (WP_DEBUG) {
					trigger_error(sprintf(__("The 'label' and 'id' values of the 'args' parameter of '%s::%s()' are required", 'esc-additional-post-thumbnails'), __CLASS__, __FUNCTION__));
				}
				return;
			}
		...
		public function add_metabox() {
			add_meta_box("{$this->post_type}-{$this->id}", __($this->label, 'esc-additional-post-thumbnails'), array($this, 'thumbnail_meta_box'), $this->post_type, $this->context, $this->priority);
		}
		...
		public function add_attachment_field($form_fields, $post) {
			...
			$link = sprintf('<a id="%4$s-%1$s-thumbnail-%2$s" class="%1$s-thumbnail" href="#" onclick="AdditionalPostThumbnails.setAsThumbnail(\'%2$s\', \'%1$s\', \'%4$s\', \'%5$s\');return false;">' . __( 'Set as %3$s', 'esc-additional-post-thumbnails' ) . '</a>', $this->id, $post->ID, $this->label, $this->post_type, $ajax_nonce);
		...
		public function enqueue_admin_scripts( $hook ) {
			...
			if (version_compare($wp_version, '3.5', '<')) {	
				add_thickbox();
				wp_enqueue_script( "esc-featured-image", _ESC_JS . 'admin/multi-post-thumbnails-admin.js', array( 'jquery', 'media-upload' ) );
			} else { // 3.5+ media modal
				wp_enqueue_media( array( 'post' => ( $post_ID ? $post_ID : null ) ) );
				wp_enqueue_script( "esc-apt-featured-image", _ESC_JS . 'admin/multi-post-thumbnails-admin.js', array( 'jquery', 'set-post-thumbnail' ) );
				wp_enqueue_script( "esc-a-featured-image-modal", _ESC_JS . 'admin/media-modal.js', array( 'jquery', 'media-models' ) );
			}
		...
		private function post_thumbnail_html($thumbnail_id = null) {
				...
				$modal_js = sprintf(
					'var mm_%3$s = new MediaModal({
						...
						cb : function(attachment){
							AdditionalPostThumbnails.setAsThumbnail(attachment.id, "%2$s", "%1$s", "%4$s");
						}
					});',
			...
			$set_thumbnail_link = sprintf( $format_string, sprintf( esc_attr__( "Set %s" , 'esc-additional-post-thumbnails' ), $this->label ), esc_url($image_library_url), $this->post_type, $this->id, $url_class, $this->label, $thumbnail_id );
			$content = sprintf( $set_thumbnail_link, sprintf( esc_html__( "Set %s", 'esc-additional-post-thumbnails' ), $this->label ) );

			if ($thumbnail_id && get_post($thumbnail_id)) {
				$old_content_width = $content_width;
				$content_width = 266;
				$attr = array( 'class' => 'apt-thumbnail' );
				...
				if (!empty($thumbnail_html)) {
					...
					$format_string = '<p class="hide-if-no-js"><a href="#" id="remove-%1$s-%2$s-thumbnail" onclick="AdditionalPostThumbnails.removeThumbnail(\'%2$s\', \'%1$s\', \'%4$s\');return false;">%3$s</a></p>';
					$content .= sprintf( $format_string, $this->post_type, $this->id, sprintf( esc_html__( "Remove %s", 'esc-additional-post-thumbnails' ), $this->label ), $ajax_nonce );
				}
...
/*	if ( is_admin() )
		load_plugin_textdomain( 'esc-additional-post-thumbnails', FALSE, basename( dirname( __FILE__ ) ) . '/languages/' );*/
}

if (class_exists('AdditionalPostThumbnails')) {
    new AdditionalPostThumbnails(
        array(
            'label' => 'Featured Image (2)',
            'id' => 'esc-featured-image-2',
            'post_type' => 'post'
        )
    );
}
/*
<?php if (class_exists('AdditionalPostThumbnails')) :
    AdditionalPostThumbnails::the_post_thumbnail(
        get_post_type(),
        'esc-featured-image-2'
    );
endif; ?>
*/